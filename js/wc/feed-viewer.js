...
class FeedViewer extends HTMLElement {
  constructor() {
    ...
    this.swipeSensitivity = 50; // pixels
    this.activePostIndex = 0;
    this.lastSwipeTime = 0; // Track last swipe time
    this.doubleSwipeThreshold = 300; // ms for double swipe detection
  }
  ...
  handleSwipe() {
    ...
    if (Math.abs(swipeDistance) > this.swipeSensitivity) {
      ...
      if (timeSinceLastSwipe < this.doubleSwipeThreshold) {
        // Accept the swipe and provide feedback
        this.provideSwipeFeedback();
        const direction = swipeDistance > 0 ? 'right' : 'left';
        this.loadData(direction);
      }

      this.lastSwipeTime = now; // Update last swipe time
    }
  }
